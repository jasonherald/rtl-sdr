...
Software-defined radio application in Rust -- a port of [SDR++](https://github.com/AlexandreRouma/SDRPlusPlus) with a native **GTK4 / libadwaita** UI. **Linux only.** (A macOS SwiftUI frontend was prototyped and later removed to keep the tree focused; it lives on the [`mac-archive`](https://github.com/jasonherald/rtl-sdr/tree/mac-archive) branch.)
...
- Secure credential storage via OS keyring (GNOME Keyring)
...
22-member Rust workspace (root binary + 21 library crates):

```text
sdr (binary)              Linux entry point
sdr-ui                    GTK4/libadwaita UI (Linux-only)
sdr-core                  Headless engine facade over the pipeline/radio stack
...
sdr-sink-audio            PipeWire audio output
...
The RTL-SDR USB driver ([`librtlsdr-rs`](https://crates.io/crates/librtlsdr-rs), a pure-Rust port of librtlsdr covering 5 tuner families over rusb) and the mDNS `_rtl_tcp._tcp.local.` discovery crate ([`sdr-rtltcp-discovery`](https://crates.io/crates/sdr-rtltcp-discovery)) were spun out as standalone published crates and are consumed from crates.io.

**Signal chain:** Source → Decimation → Channel filter → Demodulator → IF chain (squelch) → AF chain → Audio sink

**Transcription tap:** branches off AFTER the demodulator and AF filter but BEFORE volume scaling, so the recognizer always sees full-amplitude audio regardless of what the speaker side is doing. The Whisper/Sherpa path takes 48 kHz interleaved stereo.
...
Your credentials are stored in your system keyring (GNOME Keyring) and are only sent to RadioReference.com.
...
- All transcription runs on-device — Whisper or Sherpa-onnx
