...
pub(super) fn connect_source_panel(
    ...
    wire_rtl_frontend_toggles(panels, state, toast_overlay, config);

    wire_gain_and_agc_rows(panels, state, config);
    ...
    wire_rtl_tcp_client_rows(panels, state, config, &last_good_auth_key);
...
/// Split out per the 50-NLOC gate (#817).
fn wire_sample_rate_and_device_rows(
    panels: &SidebarPanels,
    state: &Rc<AppState>,
    config: &std::sync::Arc<sdr_config::ConfigManager>,
) {
...
/// Split out per the 50-NLOC gate (#817).
fn wire_iq_ppm_and_restart_rows(
    panels: &SidebarPanels,
    state: &Rc<AppState>,
    config: &std::sync::Arc<sdr_config::ConfigManager>,
) {
...
/// Split out per the 50-NLOC gate (#817).
fn wire_role_and_server_key_rows(
    panels: &SidebarPanels,
    state: &Rc<AppState>,
    config: &std::sync::Arc<sdr_config::ConfigManager>,
    favorites: &Rc<
