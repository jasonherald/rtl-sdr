//! Discovered-row rendering: the `ServerAnnounced` handler, the
//! star toggle (build + persist + reorder), stale-row pruning, the
//! Connect-button wiring, and the subtitle/age formatting helpers.
//! Split out of `window/source/discovery.rs` per the Codacy
//! large-file gate (#846).

use gtk4::prelude::*;
use libadwaita::prelude::*;

use super::super::super::{DiscoveredServer, Duration, Rc, adw, glib, sidebar};
use super::super::connect::{RtlTcpConnectRows, apply_rtl_tcp_connect};
use super::super::favorites::{
    FAVORITE_ICON_FILLED, FAVORITE_ICON_OUTLINE, FavoriteRowContext, favorite_key,
    rebuild_favorites_popover, set_favorite_toggle_accessible_name,
};
...
pub(super) fn on_server_announced(
    ...
    // `use_markup(false)`: title/subtitle are built from mDNS
    // TXT-record fields (nickname, tuner) and the advertised
    // hostname — all attacker-controlled on a hostile LAN.
    // `AdwActionRow`/`AdwPreferencesRow` default `use-markup` to
    // TRUE, so without this a crafted TXT record could inject
    // Pango markup into the discovered-servers list.
...
pub(super) fn prune_stale_discovery_rows(
    displayed_rows: &DisplayedRowsMap,
    expander: &adw::ExpanderRow,
) {
    use std::time::Instant;
...
pub(in crate::window::source) fn reorder_discovered_rows(
    expander: &adw::ExpanderRow,
...
mod rtl_tcp_discovery_format_tests {
    ...
    fn subtitle_without_ip_omits_duplicate_hostname_segment() {
        ...
        // Exactly one ` • ` separator: target + hardware/freshness.
        assert_eq!(
