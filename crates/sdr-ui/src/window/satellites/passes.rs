//! Pass-list surface: the `DisplayedPass` row records, pass-row
//! rebuild + bell / play buttons, TLE force-refresh, and the ZIP
//! ground-station lookup. Split out of `window/satellites.rs` per the
//! Codacy 500-NLOC file gate on PR #844.

use gtk4::prelude::*;
use libadwaita::prelude::*;

use super::super::{Rc, RefCell, TuneFn, adw, gio, glib, sidebar};
...
/// no notify).
pub(super) struct DisplayedPass {
    pub(super) row: adw::ActionRow,
    pub(super) pass: sdr_sat::Pass,
    pub(super) bell_btn: Option<gtk4::ToggleButton>,
}
...
pub(super) fn wire_tle_refresh_button(
