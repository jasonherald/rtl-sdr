...
pub struct KnownSatellite {
    ...
    /// soft symbols before Viterbi). METEOR-M2 4 IS differentially
    /// precoded — confirmed empirically on the first successful M2-4
    /// decode, where plain OQPSK yielded zero CADUs and only the
    /// differential chain produced imagery (#892). METEOR-M2 3 is
    /// set the same by inference (identical downlink format) and
    /// still awaits confirmation on its own live pass. Part of the
    /// downlink profile so the live decoder and the replay CLI build
    /// the same chain (#730, corrected #892). Meaningless (`false`)
    /// for non-LRPT entries.
