...
struct ChannelDsp {
    ...
    nco_step: f64,
    /// Output samples produced per input sample by [`Self::resampler`].
    out_per_in: f64,
...
impl ChannelDsp {
    fn new(freq_hz: f64, center_hz: f64, source_rate_hz: f64) -> Result<Self, OrbcommError> {
        Ok(Self {
            ...
            out_per_in: CHANNEL_SAMPLE_RATE_HZ / source_rate_hz,
            resampler: RationalResampler::new(source_rate_hz, CHANNEL_SAMPLE_RATE_HZ)?,
            fll: Fll::new(),
    ...
    /// Output slots needed for `input_len` input samples.
    ///
    ...
    fn decimated_capacity_for(&self, input_len: usize) -> usize {
        (input_len as f64 * self.out_per_in).ceil() as usize + RESAMPLER_OUTPUT_MARGIN
    }
}
...
impl ChannelBank {
    ...
    /// [`OrbcommError::Dsp`] if the resampler cannot be built for the rate pair.
    pub fn new(
        source_rate_hz: f64,
        center_hz: f64,
        channels: &[f64],
...
impl Channel {
    ...
    fn process(
        ...
        for &sample in iq {
            let (sin, cos) = dsp.nco_phase.sin_cos();
            mixed.push(sample * Complex::new(cos as f32, sin as f32));
            dsp.nco_phase = wrap_phase(dsp.nco_phase + dsp.nco_step);
        }

        // 2. Decimate to the channel rate. This is also the channel filter.
        let need = dsp.decimated_capacity_for(iq.len());
        if dsp.decimated.len() < need {
            dsp.decimated.resize(need, Complex::default());
        }
        let count = match dsp.resampler.process(mixed, &mut dsp.decimated) {
            Ok(count) => count,
            Err(error) => {
                // Unreachable given `decimated_capacity_for`, but dropping the
        ...
        dsp.fll.process(&mut dsp.decimated[..count]);

        // 4. Demodulate to information bits.
        dsp.bits.clear();
        dsp.demod.process(&dsp.decimated[..count], &mut dsp.bits);
        ...
        for &bit in &dsp.bits {
            ...
            if frame
                ...
                && let Some(message) = dsp.reassembler.push(&frame.bytes)
            {
                events.push(OrbcommEvent {
                    channel_hz: *freq_hz,
        ...
        *checksum_fail = dsp.deframer.bad_strides();
    }
}
