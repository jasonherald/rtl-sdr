...
> **⚠️ SUPERSEDED in commit `b2595ba`.** The runtime rasterize path
> below was implemented and then reverted: `librsvg`'s transitive
> dep tree dragged in ~80 crates including unmaintained `paste` and
> `fxhash` (RUSTSEC-2024-0436 / RUSTSEC-2025-0057). The shipped
> approach pre-bakes a 22×22 ARGB32 byte buffer at
> `data/com.sdr.rs.tray22.argb32` (regenerated by
> `scripts/regen-tray-icon.sh` whenever `data/com.sdr.rs.svg`
> changes), and `icon.rs` loads it via `include_bytes!`. Zero
> runtime SVG deps. **Do NOT add `librsvg` / `cairo-rs` back to
> `sdr-tray` — they pull the unmaintained transitive cluster
> straight back in.** The historical task text below is preserved
> for context only.
