...
- **`connect_doppler_tracker(panels, state, cache, status_bar)`** — runs only when the TLE cache is available. Owns the `Rc<RefCell<DopplerTracker>>` and **two `glib::timeout_add_local` timers** (the dispatch baseline lives on `AppState` — see "Dispatch baseline" below):
  ...
  **Dispatch baseline for the rate-limit gate:**
  ...
  - **Storage:** `state.last_dispatched_vfo_offset_hz: Cell<f64>` on `AppState`. Tracks the value we last asked DSP to apply, kept in sync via two complementary write paths (synchronous-on-send + echo reconciliation).
  - **Synchronous send-site updates:** `AppState::dispatch_vfo_offset(hz)` is the canonical way for any UI path to issue `UiToDsp::SetVfoOffset` — it sets the cell **before** sending the message. Used by every programmatic dispatch (auto-record AOS reset, LOS RestoreTune, mode-change reset, Doppler ticks). Without this, the cell only updates on echo round-trip, which lags AOS-side `SetVfoOffset(0.0)` and a `None → Some` fresh engagement firing in the same tick window would seed `user_reference_offset_hz` from the stale pre-AOS value.
  - **Echo reconciliation:** the `connect_vfo_offset_changed` callback also writes the cell whenever the spectrum widget's offset state changes — fires on the DSP echo (`DspToUi::VfoOffsetChanged`) and on direct spectrum-widget drag dispatches. Reconciles the optimistic synchronous write with the actually-applied value (matches harmlessly on the normal path; overwrites to truth if DSP clamped or rejected).
  - **Sync coverage:** because every VFO-mutating path either calls `dispatch_vfo_offset` (programmatic) or fires through the spectrum widget's local-update + echo path (user drag / click-to-tune), the 4 Hz tick's threshold comparison always lands against the actual current DSP state — never a stale local value.
